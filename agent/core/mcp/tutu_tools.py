...
_make_proxy_tool берёт все 16 инструментов и оборачивает их в один tutu_call(tool, args_json). 
LLM видит одну функцию с enum из 16 имён, а не 16 отдельных схем.

tutu_catalog_markdown генерирует компактную таблицу (имя + описание + список полей), которая вставляется в системный промпт — чтобы модель знала, 
какие поля у каждого инструмента.
