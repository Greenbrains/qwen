...
Интеллектуальный туристический ассистент на базе Yandex AI Studio (модели YandexGPT) и MCP‑сервера Туту.
...
- ✅ Все описания инструментов и правила MCP вынесены в скиллы (Skills Catalog)
- ✅ Агрегированное логгирование расхода токенов по агентам с записью в файл
- ✅ Динамическая настройка keep_last_turns в зависимости от сложности запроса
- ✅ Разделение моделей: лёгкая модель для роутера, мощная для специалистов
- ✅ Скилы оформлены по формату Anthropic Agent Skills (осень 2026)
- ✅ Удалены неиспользуемые файлы, код максимально упрощён и читаем
...
- Оркестратор выбирает специалиста из каталога скиллов — легко добавлять новые навыки без программирования.
- Логгирование в файл `logs.txt` с подсчётом токенов и времени выполнения (без вывода в консоль).
